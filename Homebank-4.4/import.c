 ...
 * --------------------------------------------------------------------------
 *
 ** Homebank=4.4 _ Added CSV routine for (also) importing multiple accounts.  (c)TvT_30-03-2012
 */
...
/* CSV routine for (also) importing multiple accounts --------------------  (c)TvT_12-11-2010 */
//
//      CSV-file format based on Homebank-standard
//      Added 1 line containing account-info (account-number/Bank-name) before each separate
//      list of account-transactions.
//          Format account-info line: <account-number>;<Bank-name>
//
//      !!! standard Homebank CSV is still imported as before !!!

static GList *homebank_csv_import(gchar *filename, ImportContext *ictx)
{
GtkWidget *window;					// added	(c)TvT_12-11-2010
GIOChannel *io;
GList *list = NULL;
static gint csvtype7[7] = {
					CSV_DATE,
					CSV_INT,
					CSV_STRING,
					CSV_STRING,
...
static gint csvtype2[2] = {			// added line with account-info	(c)TvT_12-11-2010
					CSV_INT,		// account-number				(c)TvT_12-11-2010
					CSV_STRING,		// account-name					(c)TvT_12-11-2010
					};
	...
	if(io != NULL)
	{
	gchar *tmpstr, *acc_num, *acc_name;     // added "acc_num" & "acc_name"	(c)TvT_12-11-2010
	gint io_stat;
	gboolean valid7, valid2;				// added "valid2"				(c)TvT_12-11-2010
	gint count = 0;
	gint error = 0;
	gint Acc_count = 0;      				// added        				(c)TvT_12-11-2010
	Account *tmp_acc, *existitem;
	Payee *payitem;
	Category *catitem;
	GError *err = NULL;
		...
		*/
		...
		for(;;)
		{
			...
			if( io_stat == G_IO_STATUS_NORMAL)
			{
				...
				Operation *newope = da_operation_malloc();
					...
					valid2 = hb_string_csv_valid(tmpstr, 2, csvtype2);			//	(c)TvT_12-11-2010
					valid7 = hb_string_csv_valid(tmpstr, 7, csvtype7);

					//DB( g_print("valid %d, '%s'\n", valid, tmpstr) );
					 
				
					// --------------------------------------------------- added	(c)TvT_12-11-2010
					if( valid2 )										        // account-info
					{
					    Acc_count++;
					       
				        str_array = g_strsplit (tmpstr, ";", 2);
						    
				        acc_num  = g_strdup(str_array[0]);
				        acc_name = g_strdup(str_array[1]);			        

					    if(( tmpstr[10] == ';' ) && ( strlen(tmpstr) < 12 ))    // only number
					    {
						    homebank_message_dialog(GTK_WINDOW(window), GTK_MESSAGE_WARNING,\
						        _("Warning"),_("Account[%d] NAME missing\n\n[acc: %d]"), Acc_count, acc_num);

						    acc_name = g_strdup_printf(_("(new_account_%d)"), Acc_count);
						}
						if( tmpstr[0] == ';' )
				        {
				            if( strlen(tmpstr) > 1 )                            // only name
				            {
						        homebank_message_dialog(GTK_WINDOW(window), GTK_MESSAGE_WARNING,\
						            _("Warning"),_("Account[%d] NUMBER missing\n\n[acc: %s]"), Acc_count, acc_name);

    						    acc_name = g_strdup_printf(_("(%s)"), acc_name);
    						}
    						else                                                // No name & number
    						{
								homebank_message_dialog(GTK_WINDOW(window), GTK_MESSAGE_ERROR,\
        						    _("Error"),_("Account[%d] ALL info missing"), Acc_count);
        						break;
    						}
						}
					}
					
					if(( !valid2 ) && ( !valid7 ))						        // Invalid line
					{
						homebank_message_dialog(GTK_WINDOW(window), GTK_MESSAGE_ERROR,\
						    _("Error"),_("CSV-file contains Invalid line _%s_"),(tmpstr));
					...
					else
					{
						...
						// --------------------------------------------- added	(c)TvT_12-11-2010
						/* account */
						if (( acc_name == NULL ) && ( acc_num == NULL ))	// Standard HomeBank CSV import
						{
                    					tmp_acc = import_create_account(accname, NULL);
						}
						else						                        // multiple accounts CSV import
						{
						    tmp_acc = da_acc_get_by_imp_name(acc_name);

						    if( tmp_acc == NULL )
						    {
				    		// check for an existing account before creating it
					    		existitem = da_acc_get_by_name(acc_name);
					    	    tmp_acc = import_create_account(acc_name, acc_num);
					    	    	
					    		if( existitem != NULL )
					    		{
					    			tmp_acc->imp_key = existitem->key;
					    		}
					    	}
					    }
						// -----------------------------------------------------------
		...
		g_free(acc_name);										//	(c)TvT_12-11-2010
		g_free(acc_num);										//	(c)TvT_12-11-2010
